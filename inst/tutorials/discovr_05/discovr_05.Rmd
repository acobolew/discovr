...
author:
  - name: "Andy Field"
    role: "everything but sina and the parts combining violins with boxplots"
  - name: "Alan Cobo-Lewis"
    role: "sina plots, combining violins with boxplots"
output:
  learnr::tutorial:
    progressive: false
    theme: "united"
...
description: "Visualizing data. The ggplot2 package, boxplots, plotting means, violin plots, sina plots, scatterplots, grouping by colour, grouping using facets, adjusting scales, adjusting positions."
...
## `r bmu()` Violins with boxplots [(1)]{.alt}

The original conception of a violin plot was to display the distribution of the data (the violin) along with the information in a boxplot @hintze1998., but violin plots are currently regarded in most circles as showing the just the distribution (the "violin" part). But ggplot2 makes it easy to add a boxplot layer along with a violin layer, which let's the graph show the full distribution while also highlighting the quartiles (median and interquartile range).

#### `r robot()` Code example

Recall a simple boxplot of the Jiminy Cricket data:
...
We can add a violin underneath the boxplot by adding `geom_violin() +` before `geom_boxplot()`:
...
This is on the right track, but the problem is that the boxes are too wide, so they obscure part of the violins.

#### `r alien()` Alien coding challenge

Adjust the boxplot so that it's narrow enough that it doesn't obscure the violin. You can accomplish this by using the `width=` argument of `geom_boxplot()`. Try different values of the width argument to make this work. You can use values smaller than 1, expressed as decimals like 0.5 or as fractions like 1/2.

```{r wish_violin_box_3, exercise = TRUE, exercise.lines = 6}
wish_plot <- ggplot2::ggplot(wish_tib, aes(time, success))
wish_plot +
  geom_violin() +
  geom_boxplot() +  
...
```{r wish_violin_box_3-hint-1}
wish_plot <- ggplot2::ggplot(wish_tib, aes(time, success))
wish_plot +
  geom_violin() +
  geom_boxplot(width=xxxx) + # try different values for width
  labs(x = "Time", y = "Success (%)") +
  theme_minimal()
```

```{r wish_violin_box_3-solution}
wish_plot <- ggplot2::ggplot(wish_tib, aes(time, success))
wish_plot +
  geom_violin() +
  geom_boxplot(width=.125) +
  labs(x = "Time", y = "Success (%)") +
  theme_minimal()
```

## `r bmu()` Sina plots [(1)]{.alt}

Similar to violin plots are sina plots, named after the person who first coded them, Sina Hadi Sohi (a graduate student at the time!). A sina plot is like a "one-dimensional scatterplot" that shows all the underlying data points, but spread apart so that, collectively, they cover the same region as would be covered by a violin. A sina layer can be added with `geom_sina()`, which is a function in the `ggforce` package.

#### `r robot()` Code example

```{r wish_sina_violin_box}
wish_plot <- ggplot2::ggplot(wish_tib, aes(time, success))
wish_plot +
  ggforce::geom_sina() +
  labs(x = "Time", y = "Success (%)") +
...
Unless there are a very large amount of data points, it's usually easier to see the shape of the distribution if a sina plot is combined with a violin plot.

#### `r alien()` Alien coding challenge

Try adding a violin layer--be sure to put before the sina layer so that it doesn't overwrite the datapoints in the sina.

```{r wish_sina_violin_box_2, exercise = TRUE, exercise.lines = 6}
wish_plot <- ggplot2::ggplot(wish_tib, aes(time, success))
wish_plot +
  ggforce::geom_sina() +
  labs(x = "Time", y = "Success (%)") +
...
```{r wish_sina_violin_box_2-hint-1}
wish_plot <- ggplot2::ggplot(wish_tib, aes(time, success))
wish_plot +
  geom_violin() +
  ggforce::geom_sina() +
...
## `r bmu()` Sina + violin + box [(1)]{.alt}

You can also add boxplots to sina plots (or all three together). But when you do that, you'll want to suppress the outliers on the boxplot, because otherwise they'll get double-plotted (plotted once by the sina and plotted again by the boxplot). You can accomplish that by using the argument `outlier.shape=NA` in `geom_boxplot()`.

In addition, when combining boxplots with sina plots--and especially when combining all 3--it probably makes sense to suppress the boxplot's whiskers. By default, `geom_boxplot()` extends the whiskers to 1.5 times the interquartile range. So suppress the whiskers entirely, make the whiskers only 0 times the interquartile range (tricky, eh?).

#### `r robot()` Code example

```{r wish_sina_violin_box_3}
wish_plot <- ggplot2::ggplot(wish_tib, aes(time, success))
wish_plot +
  geom_violin() +
  ggforce::geom_sina() +
  geom_boxplot(width = 1/8, outlier.shape = NA, coef = 0) +
  labs(x = "Time", y = "Success (%)") +
  theme_minimal()
```
