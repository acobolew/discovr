...
We can visualise the data easily using the `GGally` package, which we met in `discovr_07`. When you want to plot continuous variables, the `ggpairs()` function from this package produces a matrix of scatterplots (below the diagonal), distributions (along the diagonal) and the correlation coefficient (above the diagonal). It takes the general form:
...
```{r album_plot-hint-3}
# solution
GGally::ggpairs(album_tib, columns = c("adverts", "airplay", "image", "sales")) +
  theme_minimal()
```
